...
featuredImage: "https://media.internetinmyanmar.com/images/ooni-probe-myanmar-censorship-data/ooni-probe-myanmar-test-type-chart-1.png"
featuredImageAlt: "OONI anomaly rates for messaging apps and circumvention tools in Myanmar"
lang: "en"
sources:
  - "https://ooni.org"
...
import OoniTestTypeChart from '../../components/charts/OoniTestTypeChart.astro'
...
import OoniCoverageChart from '../../components/charts/OoniCoverageChart.astro'
...
Myanmar's censorship apparatus doesn't just target websites. The test-type breakdown above shows that messaging apps and circumvention tools are blocked almost as systematically as web content — closing both the information channel and the escape route simultaneously. Signal, Telegram, and WhatsApp face the same network-layer interference as independent news sites; Tor and Psiphon, the standard tools used to tunnel past those blocks, are themselves blocked under the 2025 Cybersecurity Law.

This pattern is not accidental. It is the signature of censorship that has become structural. Myanmar's junta no longer issues emergency block orders against specific news events; those blocks are already in place, applied at the network layer, and maintained continuously by ISPs operating under military directive. The [2025–26 election period](/articles/myanmar-election-internet-censorship-2025-2026/) produced no measurable anomaly spike precisely because the information environment was already comprehensively controlled before the first ballot was cast.
...
The category breakdown above maps OONI's anomaly data to the Citizen Lab content taxonomy — the same classification system used by researchers globally to compare censorship across countries. The pattern in Myanmar is not random or reactive. News & Media, Political Content, Human Rights, and Anonymization Tools all show consistently elevated anomaly rates, reflecting systematic targeting by policy category rather than individual editorial decisions. LGBTQ+ content is blocked at rates that have no equivalent in Myanmar's pre-coup internet policy. The category chart makes visible what domain-level data can obscure: that Myanmar's censorship is shaped by deliberate political choices about which categories of expression the state wishes to control.
...
The chart above maps monthly probe measurement volume against anomaly rate from the coup through 2026. Where the bars are thin, coverage is sparse — and the anomaly rate is less reliable. The February 2021 surge reflects an influx of volunteer probes immediately after the coup; the subsequent decline reflects attrition under digital security pressure. The gap between what probes can see and what is actually happening on blacked-out networks widens as coverage falls.
